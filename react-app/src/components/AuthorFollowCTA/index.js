...
const AuthorFollowCTA = ({ author, user, following, onFollow, followedCount }) => {
  ...
  if (!author || !user || user.id === author.id || dismissed) return null;
  ...
  return (
    <div className="author-cta">
      <button className="author-cta-dismiss" onClick={() => setDismissed(true)} aria-label="Dismiss">
        ✕
      </button>
      <div className="author-cta-body">
        ...
        <div className="author-cta-info">
          {followedCount > 0 && (
            <p className="author-cta-context">Following {followedCount} author{followedCount !== 1 ? 's' : ''}.</p>
          )}
          <p className="author-cta-name">{authorName}</p>
          <p className="author-cta-followers">{author.numFollowers} follower{author.numFollowers !== 1 ? 's' : ''}</p>
        </div>
      </div>
      <button
        ...
        onClick={onFollow}
      >
        {following ? 'Following' : `Follow ${author.firstName}`}
      </button>
    </div>
  );
};
